...
def load_dataset(args):
	...
	def _get_transform(args, train=False):
		transform = torchvision.transforms.Compose(
			[
				...
				torchvision.transforms.ColorJitter(brightness=args.randjit, contrast=args.randjit) if (
						args.randjit is not None and train) \
					else torchvision.transforms.Lambda(lambda x: x),
	...
	def _construct_dataset(raw_train, idx, sample_indices):
		...
		if args.num_classes is None:  # regression
			training_set, test_set = torch.utils.data.random_split(subset,
			                                                       [len(subset) - int(len(subset) * args.test_size),
			                                                        int(len(subset) * args.test_size)])
		else:  # classification
	...
	if args.use_model_tokenizer:
		assert args.model_name.lower() in transformers.models.__dict__.keys(), \
			f'Please check if the model (`{args.model_name}`) is supported by `transformers` module!'
	...
	if args.dataset in ['FEMNIST', 'Shakespeare', 'Sent140', 'CelebA',
	                    'Reddit']:  # 1) for a special dataset - LEAF benchmark...
		...
		if args.dataset in ['FEMNIST', 'CelebA']:
			...
			if args.crop is None:
				logger.info(
					f'[LOAD] Dataset `{args.dataset}` may require `crop` argument; (recommended: `FEMNIST` - 28, `CelebA` - 84)!')
			transforms = [_get_transform(args, train=True), _get_transform(args, train=False)]
		elif args.dataset == 'Reddit':
			args.rawsmpl = 1.0
		
		# construct split hashmap, client datasets
		# NOTE: for LEAF benchmark,
		# values of `split_map` hashmap is not indices, but sample counts of tuple (training set, test set)!
		...
		transforms = [_get_transform(args, train=True), _get_transform(args, train=False)]
		raw_train, raw_test, args = fetch_torchvision_dataset(args=args, dataset_name=args.dataset, root=args.data_path,
		                                                      transforms=transforms)
	
	elif args.dataset in torchtext.datasets.__dict__.keys():  # 4) for downloadable datasets in `torchtext.datasets`...
		_check_and_raise_error(args.split_type, 'pre', 'split scenario')
		raw_train, raw_test, args = fetch_torchtext_dataset(args=args, dataset_name=args.dataset, root=args.data_path,
		...
		if parsed_type in ['A', 'L']:
			...
			logger.exception(err)
			raise Exception(err)
		raw_train, raw_test, args = fetch_beerreviews(args=args, root=args.data_path, aspect=aspect,
		                                              tokenizer=tokenizer)
	
	elif args.dataset == 'Heart':
		_check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
		_check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
		split_map, client_datasets, args = fetch_heart(args=args, root=args.data_path, seed=args.seed,
		                                               test_size=args.test_size)
	
	elif args.dataset == 'Adult':
		_check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
		_check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
		split_map, client_datasets, args = fetch_adult(args=args, root=args.data_path, seed=args.seed,
		                                               test_size=args.test_size)
	
	elif args.dataset == 'Cover':
		_check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
		_check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
		split_map, client_datasets, args = fetch_cover(args=args, root=args.data_path, seed=args.seed,
		                                               test_size=args.test_size)
	
	elif args.dataset == 'GLEAM':
		_check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
		_check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
		split_map, client_datasets, args = fetch_gleam(args=args, root=args.data_path, seed=args.seed,
		                                               test_size=args.test_size, seq_len=args.seq_len)
	...
	if args.eval_type == 'local':
		...
		if raw_test is None:
			err = (f'[LOAD] Dataset '
			       ...
			       f'does not support pre-defined validation/test set, '
			       f'which can be used for `global` evluation... '
			       f'please check! (current `eval_type`=`{args.eval_type}`)')
			logger.exception(err)
			raise AssertionError(err)
	...
	if client_datasets is None:
		...
		if (args.eval_type == 'local') and (args.test_size == -1):
			holdout_sets = torch.utils.data.random_split(_raw_test,
			                                             [int(len(_raw_test) / args.K) for _ in range(args.K)])
			holdout_sets = [SubsetWrapper(holdout_set, f'< {str(idx).zfill(8)} > (test)')
			                for idx, holdout_set in enumerate(holdout_sets)]
			augmented_datasets = []
